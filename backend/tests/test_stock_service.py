...
class TestStockService:
    ...
    def test_get_stock_data_success(self, stock_service, mock_yfinance_ticker):
        """Test successful stock data retrieval"""
        with patch('yfinance.Ticker', return_value=mock_yfinance_ticker):
            result = stock_service.get_stock_data('AAPL', '2025-11-01', '2025-11-06')
    ...
    def test_get_stock_data_invalid_symbol(self, stock_service, mock_empty_ticker):
        ...
        with patch('yfinance.Ticker', return_value=mock_empty_ticker):
            with pytest.raises(ValueError, match="No data found"):
                stock_service.get_stock_data('INVALID', '2025-11-01', '2025-11-06')

    def test_get_stock_data_network_error(self, stock_service):
        ...
        with patch('yfinance.Ticker') as mock_ticker:
            ...
            with pytest.raises(ValueError, match="Failed to fetch stock data"):
                stock_service.get_stock_data('AAPL', '2025-11-01', '2025-11-06')

    def test_get_company_name_with_mapping(self, stock_service):
        """Test company name retrieval with pre-defined mapping"""
        result = stock_service.get_company_name('AAPL')

        assert isinstance(result, dict)
        assert 'zh-TW' in result
        assert 'en-US' in result

    def test_get_company_name_from_ticker_info(self, stock_service, mock_yfinance_ticker):
        """Test company name extraction from ticker info"""
        result = stock_service.get_company_name(
            'TEST',
    ...
    def test_data_point_conversion(self, stock_service, mock_yfinance_ticker):
        """Test conversion of DataFrame to data points"""
        with patch('yfinance.Ticker', return_value=mock_yfinance_ticker):
            result = stock_service.get_stock_data('TEST', '2025-11-01', '2025-11-06')
    ...
    def test_result_includes_price_info(self, stock_service, mock_yfinance_ticker):
...
class TestStockServiceErrorHandling:
    ...
    def test_empty_history_response(self, stock_service, mock_empty_ticker):
        """Test when ticker.history() returns empty DataFrame"""
        with patch('yfinance.Ticker', return_value=mock_empty_ticker):
            with pytest.raises(ValueError, match="No data found"):
                stock_service.get_stock_data('EMPTY', '2025-11-01', '2025-11-06')

    def test_none_history_response(self, stock_service):
        ...
        with patch('yfinance.Ticker', return_value=mock_ticker):
            with pytest.raises(ValueError):
                stock_service.get_stock_data('NONE', '2025-11-01', '2025-11-06')

    def test_yfinance_timeout_error(self, stock_service):
        ...
        with patch('yfinance.Ticker', return_value=mock_ticker):
            with pytest.raises(ValueError, match="Failed to fetch stock data"):
                stock_service.get_stock_data('TIMEOUT', '2025-11-01', '2025-11-06')

    def test_yfinance_connection_error(self, stock_service):
        ...
        with patch('yfinance.Ticker', return_value=mock_ticker):
            with pytest.raises(ValueError):
                stock_service.get_stock_data('CONN_ERR', '2025-11-01', '2025-11-06')


class TestStockServiceDataConversion:
    """Test data conversion and validation"""

    def test_convert_with_nan_values(self, stock_service):
        ...
        with patch('yfinance.Ticker', return_value=mock_ticker):
            ...
            try:
                result = stock_service.get_stock_data('NAN_TEST', '2025-11-01', '2025-11-03')
    ...
    def test_volume_type_conversion(self, stock_service, mock_yfinance_ticker):
    ...
    def test_zero_volume_handling(self, stock_service):
        ...
        with patch('yfinance.Ticker', return_value=mock_ticker):
            result = stock_service.get_stock_data('ZERO_VOL', '2025-11-01', '2025-11-03')
...
class TestStockServiceCompanyNameResolution:
    ...
    def test_get_company_name_fallback_logic(self, stock_service):
        """Test shortName → longName → None fallback chain"""
        # Test with only longName
        ticker_info = {'longName': 'Test Company Long Name'}
        result = stock_service.get_company_name('TEST', ticker_info=ticker_info)

        assert isinstance(result, dict)
        assert result['en-US'] == 'Test Company Long Name'

    def test_get_company_name_info_access_error(self, stock_service):
        ...
        with patch('yfinance.Ticker', return_value=mock_ticker):
            ...
            try:
                result = stock_service.get_company_name('TEST')
                assert isinstance(result, dict)
            except Exception:
                # If it raises, should be handled appropriately
                pass

    def test_get_company_name_empty_info(self, stock_service):
        """Test when ticker.info is empty dict"""
        result = stock_service.get_company_name('TEST', ticker_info={})
...
class TestStockServiceEdgeCases:
    ...
    def test_special_characters_in_symbol(self, stock_service):
    ...
    def test_very_large_volume(self, stock_service, mock_yfinance_ticker):
    ...
    def test_date_range_validation(self, stock_service, mock_yfinance_ticker):
        ...
        with patch('yfinance.Ticker', return_value=mock_yfinance_ticker):
            ...
            result2 = stock_service.get_stock_data('AAPL', '2025-11-05', '2025-11-05')
            assert 'data' in result2

    def test_malformed_data_structure(self, stock_service):
        ...
        with patch('yfinance.Ticker', return_value=mock_ticker):
            ...
            try:
                stock_service.get_stock_data('MALFORMED', '2025-11-01', '2025-11-02')
            except (ValueError, KeyError, AttributeError):
                # Acceptable to raise error for malformed data
                pass

    def test_future_date_handling(self, stock_service, mock_yfinance_ticker):
        """Test that future dates are handled gracefully"""
        with patch('yfinance.Ticker', return_value=mock_yfinance_ticker):
            # Future dates should either return empty data or current data
            result = stock_service.get_stock_data('AAPL', '2026-01-01', '2026-01-05')
