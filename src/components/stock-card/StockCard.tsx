...
const StockCard = memo(function StockCard({
  ...
  const handleNewsPanelClose = useCallback(() => {
  ...
  return (
    <div
      className={`group/card h-full max-h-[220px] overflow-hidden p-3 flex flex-col transition-all duration-300 ease-out ${
        visualTheme === "warm"
    ...
    >
      ...
      {/* Volume chart */}
      <StockVolumeChart data={stockData.data} t={t} isVisible={true} />
      ...
      {/* News Panel (rendered via portal) */}
      <NewsPanel
        isOpen={isNewsPanelOpen}
        onClose={handleNewsPanelClose}
        symbol={stockData.symbol}
      />
    </div>
  );
